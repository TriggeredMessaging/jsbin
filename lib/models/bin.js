...
module.exports = Observable.extend({
  ...
  tmsDeleteBin: function (url, fn) {
      var timer = metrics.createTimer(metric_prefix + 'tmsDelete');
      this.store.tmsDeleteBin(url, function (err, result) {
          if (err) {
              return fn(err);
          }

          timer.stop();
          fn(err || null, err ? undefined : result);
      }.bind(this));
  }
});
