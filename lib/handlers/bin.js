...
module.exports = Observable.extend({
  ...
  tmsDeleteBin: function (req, res, next) {
      var url = req.params.bin;
      this.models.bin.tmsDeleteBin(url, function (err, result) {
          if (err) {
              res.json({deleted: false, reason: err});
          } else {
              res.json({deleted: true, result: result});
          }
      });
  }
});
